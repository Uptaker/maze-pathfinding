<script lang="ts">
  ...
  export let finishPosition: Position
  ...
  async function solve() {
    ...
    while (queue.length > 0 && !pathFound) {
      let index = findShortestDistanceIndex(queue)
      pos = queue[index]
  ...
  function findShortestDistanceIndex(queue: Position[]) {
    let shortestDistance = Infinity;
    let shortestIndex = 0
    for (let i = 0; i < queue.length; i++) {
      let distance = distSquared(finishPosition, queue[i])
  ...
  function distSquared(pos1: Position, pos2: Position) {
